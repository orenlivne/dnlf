...
applicable even to a commodity that concentrates on few arcs. We fall back to a direct solve for the very rare inputs on which the AC factorization throws. The inner conjugate-gradient count is single digit at and near
...
at that $\gamma$ and do not pursue $\gamma\to0$. We verified that a coupling-aware preconditioner
(per-arc coupling inverted by Sherman--Morrison, aggregate-congestion Laplacian on the mean commodity mode)
converges $\gamma$-boundedly where the reduced-potential version does not, but
