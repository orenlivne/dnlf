...
sensitivity-analysis (SAB) methods, whose every step factorizes the equilibrium Jacobian (cubic in the active
subnetwork). We answer it directly on the CAIDA router-level Internet graph ($n=190{,}914$, $m=1.22\times10^6$),
...
are the two operations an SAB step performs by cubic factorization; here both are near-linear, so a design
...
\subsection{Crossover on Size-Scaled Networks}
...
$1.1\times10^4$ to $6.0\times10^6$ arcs ($2.74$ decades). Timing was serial and uncontended. \emph{Every}
instance converges: the equilibrium residual is at most $2.7\times10^{-3}$ (median $7.9\times10^{-5}$; $90$ of
$96$ below $10^{-3}$, $23$ at machine precision), with the inner-engine setup count in $[47,330]$---bounded, and
set by conditioning rather than size. The wall-clock fits $t\propto m^{1.10}$ over the full corpus and
$t\propto m^{0.97}$ on the $m\ge10^5$ subset ($56$ graphs), where the size-independent per-solve overhead (a
fixed continuation cost of ${\approx}30$~s) no longer flattens the low end---i.e.\ essentially linear in the
regime where the asymptotics are visible, consistent with the $m^{1.17}$ of the controlled synthetic family
(\cref{tab:scaling}). This corpus power law is descriptive, not a complexity proof: at a fixed size the solve
...
$1.1\times10^4$ to $6.0\times10^6$ arcs ($2.74$ decades). Wall-clock fits $t\propto m^{1.10}$ over the full
range and $t\propto m^{0.97}$ on the $m\ge10^5$ subset (where the fixed ${\approx}30$~s continuation overhead no
longer flattens the low end), with a \emph{bounded} inner-engine setup count in $[47,330]$. Every instance
converges (residual median $7.9\times10^{-5}$, max $2.7\times10^{-3}$)---a descriptive near-linear trend across
real graphs of every family, not a complexity proof (\cref{sec:results}).}\label{fig:corpus}\end{figure}
...
same exponent, $m^{1.17}$ and track within a few percent; direct scales as $m^{1.48}$ and is slower across the
...
inner count stays single-digit at and near equilibrium across all $\gamma$---the regime the warm-started design
loop lives in (\cref{tab:mcdesign}). Only the \emph{cold} solve at small $\gamma$, far from equilibrium where
...
established on the synthetic family, \cref{tab:mc}). What sets the count is not size but the per-arc commodity coupling
