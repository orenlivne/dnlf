...
that sets each arc's cost, a per-arc Sherman--Morrison (block-diagonal-plus-rank-one) coupling. In the commodity
...
the Laplacians differ the deviations mix in as well, at a rate set only by their spread. Empirically (\cref{tab:mc}) the inner conjugate-gradient count is single digit to low double digit,
...
fallback for the rare inputs on which the approximate-Cholesky factorization throws. The count is single digit at and near
