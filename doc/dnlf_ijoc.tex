...
the equilibrium solve across a topology-diverse corpus of $20$ irregular
SuiteSparse/SNAP graphs---autonomous-system, peer-to-peer, collaboration, email, signed-social,
wiki-voting, and messaging networks (peer-to-peer and collaboration the largest groups), spanning
$2.5\times10^4$ to $3.9\times10^5$ arcs (${\approx}1.2$ decades).  Timing was serial and uncontended. The wall-clock fits
$t\propto m^{0.90}$ and the inner-engine setup count stays bounded across the entire corpus
(\cref{fig:corpus}), consistent with the $m^{1.17}$ and bounded setups of the controlled synthetic family
(\cref{tab:scaling}). Note that this corpus power law is descriptive, not a complexity estimate: at a fixed size the solve
time varies several-fold with separator structure (a peer-to-peer overlay takes ${\sim}2\times$ an
autonomous-system graph of equal edge count), which broadens the scatter.
...
$20$ irregular SuiteSparse/SNAP graphs (seven families; peer-to-peer and collaboration largest, signed-social
and messaging single instances). Wall-clock fits $t\propto m^{0.90}$ over ${\approx}1.2$ decades, with a \emph{bounded}
inner-engine setup count (in $[87,311]$, one build per fine $\delta$-level)---a descriptive near-linear trend across real graphs of every family, not a
complexity estimate (\cref{sec:results}).}\label{fig:corpus}\end{figure}
...
descriptive $m^{0.90}$ fit of \cref{fig:corpus}. (iii) \textbf{One hierarchy across design steps.} Each exact
