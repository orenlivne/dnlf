...
\For{$\ell=0,\dots,L$} \Comment{smoothing homotopy: one hierarchy build per level}
  ...
  (a few steps, warm-starting the next level), else to the tight $\varepsilon$; hierarchy \textbf{frozen} within the level
\EndFor
\State \textbf{polish:} chord-Newton on the exact law $B\rho(B^\top\phi-\tau)=d$ to the tight tolerance $\varepsilon$, warm-started from $\phi$,
