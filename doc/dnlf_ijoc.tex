...
We corroborate this on real-world irregular networks from the SuiteSparse/SNAP collection~\citep{leskovec2007}---Internet
autonomous-system graphs and peer-to-peer overlays, the topologies of the selfish-routing application
(\cref{tab:corpus}; at these sub-crossover sizes we use AC). The inner-engine setup count is size-independent ($14$--$20$) across the corpus, confirming
NLF scales linearly on real graphs. The head-to-head against the fair direct baseline is genuinely
instance-dependent at these accessible sizes ($n\le2.6\times10^4$): the NLF solve is $1.5$--$1.6\times$
...
edge count alone. NLF is already competitive-to-faster on the actual application
...
smaller head-to-head of \cref{tab:corpus} instead uses AC to face the direct baseline.
