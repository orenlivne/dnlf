...
sequence of near-linear-time Laplacian solves. The one obstacle, that the rectified dead zone makes the Jacobian
...
toll design matches the best scaled marginal-cost (Pigouvian) toll to within near-stationarity. The wall-clock scales near-linearly in the edge count---confirmed on a corpus of $96$ real irregular graphs up to $6\times10^6$ arcs---while direct factorizations scale superlinearly, so on large irregular congestion networks NLF is faster for graphs of $\approx 10,000$ arcs or more, and reaches sizes where direct factorization exhausts memory. The design inherits this
near-linear per-step cost. On near-planar road networks, where nested-dissection factorization is already
near-optimal, NLF does not win; the advantage is on poorly-separable networks, e.g., selfish
...
road networks, destination-bundled design cuts total system travel time by $7.7$--$13.1\%$. Code is available at \url{https://github.com/orenlivne/dnlf}.
}
