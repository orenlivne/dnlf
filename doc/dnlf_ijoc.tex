...
\paragraph{Design at scale.} The reviewer-facing question is not the size of any one TSTT reduction but whether
the \emph{per-step} design cost stays near-linear far beyond the ${\sim}10^2$-link ceiling of exact
sensitivity-analysis (SAB) methods, whose every step factorizes the equilibrium Jacobian (superlinearly, fill-dominated). We answer it directly on the CAIDA router-level Internet graph ($n=190{,}914$, $m=1.22\times10^6$),
a genuine communication network in the paper's target regime. There the cold equilibrium solves in $149$~s, and
each subsequent design step costs \emph{one} adjoint solve---which returns the gradient with respect to
\emph{all} $1.22\times10^6$ tolls in $\mathbf{0.9}$~s via a single near-linear Laplacian solve
(\cref{prop:adjoint})---plus \emph{one} warm equilibrium re-solve at ${\approx}30$~s. The adjoint and re-solve
are the two operations an SAB step performs by direct factorization; here both are near-linear, so a design
iteration on a $1.2$-million-link network costs seconds, not the hours (or out-of-memory) a direct
factorization at this size would require. The TSTT \emph{reduction} realized is instance-dependent (a function
of the congestion regime) and is quantified on the calibrated road networks---$5.15\%$ on Sioux Falls above and
$7.7$--$13.1\%$ on Anaheim (\cref{sec:mc}); the point here is that the machinery that produces those reductions
runs, unchanged, three-to-four orders of magnitude larger than exact SAB design can reach.
