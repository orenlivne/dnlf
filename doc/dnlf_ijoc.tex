...
nested-dissection Cholesky (CHOLMOD$+$METIS)---scale superlinearly, so on large irregular congestion networks NLF is faster
beyond a small setup-dominated crossover, by a margin set by the exponent gap, and reaches sizes where direct factorization exhausts memory; the design inherits this
...
surveys~\citep{farahani2013} catalog SAB descent, MPEC, and metaheuristic methods, with second-best tolling
treated by \citet{lawphongpanich2004} and the economic theory of road pricing by \citet{yanghuang2005}.
Equilibrium sensitivity originates with \citet{tobinfriesz1988} and was sharpened by \citet{qiumagnanti1989}
and by Patriksson and co-authors~\citep{jp2007}, who observed that the sensitivity problem is itself a
linearized equilibrium solve.
...
for arc $a$ from tail $t(a)$ to head $h(a)$. Separable BPR latencies are $t_a(f)=t^0_a\big(1+\beta_a(f/\kappa_a)^{p_a}\big)$
...
the stall. The corpus study (\cref{sec:corpus}) uses this loose-homotopy, adaptive-polish combination: it
...
every step (superlinearly, fill-dominated). The reported literature~\citep{tobinfriesz1988,jp2007,chiou2005} caps
this near ${\sim}10^2$ links, but the operative modern bound is the memory wall of the fill itself: even the fair
CHOLMOD$+$METIS factorization we measure in \cref{sec:results} exhausts RAM at $m{\approx}6.4\times10^5$. The
...
CHOLMOD$+$METIS $m^{1.24}$ overall ($m^{1.43}$ once the factorization dominates, $m\ge8\times10^4$). The
nested-dissection Cholesky is the stronger direct solver---faster than UMFPACK at all but one (anomalous,
$m{=}4\times10^4$) size ($147$ vs.\ $273$~s at $m{=}1.6\times10^5$) and reaching twice as far before its memory
wall---yet NLF is faster than \emph{both} beyond the smallest, setup-dominated point (where the
three agree to within ${\approx}2\%$); the margin over the fair baseline is $1.12\times$ at $m{=}1.6\times10^5$ and
$1.79\times$ at $m{=}3.2\times10^5$, though the latter is inflated by single-run variance at the largest sizes (the
multi-seed exponent spread is $[1.06,1.23]$, below), so the decisive evidence is the exponent gap---near-linear
vs.\ $m^{1.43}$---and the memory wall, not the point margin. Both direct solvers exhaust RAM where NLF
...
${\approx}730$~s and scales to $6\times10^6$ arcs on the corpus (\cref{sec:corpus}). The near-linear-vs.-superlinear
...
run and consistent with the $96$-graph corpus fit $t\propto m^{1.10}$ (\cref{sec:corpus}; bootstrap CI
...
interchangeable; we use LAMG$+$ by default for its robustness on large graph instances.

\subsection{Corpus Scaling}\label{sec:corpus}
...
budget (median $7.3\times10^{-5}$; $90$ of $96$ below $10^{-3}$, $23$ below $10^{-6}$; the remaining $6$,
...
Inner-engine setup count is $[47,330]$, weakly size-correlated ($r=0.20$). Residual median $7.3\times10^{-5}$,
...
$m{\approx}3.2\times10^5$; CHOLMOD$+$METIS $m^{1.24}$ overall ($m^{1.43}$ for $m\ge8\times10^4$), reaching twice as
...
stronger CHOLMOD$+$METIS ($m^{1.24}$)---each of which exhausts memory (dotted walls) where NLF continues
...
that scale remains to be shown. Reaching many commodities is limited by the $\mathcal{O}(Km)$ cost of the $K$
per-commodity block solves per step; since the commodities couple only through the low-rank, shared-arc
congestion term (\cref{prop:mc}), a promising direction is to aggregate destinations of similar arc-footprint
into $K'\ll K$ \emph{super-commodities} used as a deflation space for the block-Newton system, reducing the
effective block-solve count toward the numerical rank of the coupling where footprints cluster (to be assessed
against specialized multicommodity interior-point solvers~\citep{castro2000}). (ii) \textbf{Accuracy on the most hub-dominated graphs.} On a handful of extreme hub graphs (dense web crawls)
...
\bibitem[Qiu and Magnanti(1989)]{qiumagnanti1989} Y.~Qiu and T.~L.~Magnanti, \emph{Sensitivity analysis for variational inequalities defined on polyhedral sets},
Mathematics of Operations Research, 14 (1989), pp.~410--432.
\bibitem[Lawphongpanich and Hearn(2004)]{lawphongpanich2004} S.~Lawphongpanich and D.~W.~Hearn, \emph{An MPEC approach to second-best toll pricing},
Mathematical Programming, 101 (2004), pp.~33--55.
\bibitem[Yang and Huang(2005)]{yanghuang2005} H.~Yang and H.-J.~Huang, \emph{Mathematical and Economic Theory of Road Pricing}, Elsevier, 2005.
\bibitem[Castro(2000)]{castro2000} J.~Castro, \emph{A specialized interior-point algorithm for multicommodity network flows},
SIAM Journal on Optimization, 10 (2000), pp.~852--877.
