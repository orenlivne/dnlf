...
and web networks (peer-to-peer and collaboration the largest groups), spanning
...
time varies several-fold with separator structure (a word-association graph takes ${\sim}3\times$ a
collaboration graph of equal edge count), which broadens the scatter.

Six of these corpus graphs we also run head-to-head against a direct sparse-factorization baseline in the loose-continuation mode ($\sim10^{-3}$ residual, where a single frozen factorization per level keeps the direct solve affordable; the fine-continuation $10^{-9}$ solve of \cref{tab:scaling} instead does ${\sim}100$ inner solves), \cref{tab:corpus}. The comparison is instance-dependent: the NLF solve is $1.5$--$1.6\times$ faster on the sparsest overlay/AS graphs (\texttt{p2p-Gnutella08}, \texttt{Oregon-1}), essentially tied on \texttt{as-caida}, and slower on the smallest (\texttt{as-735}) and the denser \texttt{p2p-Gnutella24}, where a good fill-reducing ordering still wins. Which solver wins is set by the graph's separator structure, not its edge count alone; NLF is competitive-to-faster on the actual application topologies, with the advantage concentrated on the sparsest, least-separable instances.
...
$20$ irregular SuiteSparse/SNAP graphs (seven families; collaboration and peer-to-peer largest, web and citation
single instances). Wall-clock fits $t\propto m^{0.90}$ over ${\approx}1.2$ decades, with a \emph{bounded}
...
component): Internet AS graphs and P2P overlays. Near-linear AC vs.\ the fair
frozen-per-level direct baseline; inner-engine \emph{setups} stay size-independent ($14$--$20$ across the corpus).
Single run on one workstation; AC is randomized, so wall-clock and setup counts vary
$\pm$($10$--$30$)\% run to run; the qualitative winner per graph is still stable.
Dash: direct capped at $m>1.5\times10^5$.}
\label{tab:corpus}
\begin{tabular}{lrrrrr}\toprule
graph & $n$ & $m$ & AC & direct & setups\\\midrule
...
block diagonal---one near-linear (AC) solve per commodity. The rank-one commodity coupling is
