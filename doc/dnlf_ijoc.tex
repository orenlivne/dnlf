...
polish. On Sioux Falls, NLF matches Frank--Wolfe, the adjoint matches finite differences, and toll design reaches the exact marginal-cost optimum. The wall-clock scales near-linearly in the edge count while a direct factorization scales superlinearly, so on large \emph{irregular} congestion networks NLF overtakes direct factorization at a crossover, and the design inherits this near-linear per-step cost. On near-planar road networks, where nested-dissection factorization is already near-optimal, NLF does \emph{not} win; the advantage is on poorly-separable networks such as those modeling selfish routing. We extend the framework to the \emph{multicommodity} logit stochastic user equilibrium, where a block preconditioner solves the coupled block-Laplacian Newton system in a handful of iterations and the design adjoint again returns all toll gradients in one solve; on the Sioux Falls and Anaheim road networks, destination-bundled design cuts total system travel time by $7.7$--$13.1\%$. Code is available at \url{https://github.com/orenlivne/dnlf}.
}
...
Laplacian with no dead zone. We continue $\delta$ geometrically with a \emph{fine} ratio ($0.85$), from
...
On size-scaled irregular (poorly-separable) congested networks the fine-continuation solve reaches a $10^{-9}$ equilibrium residual with a wall-clock that scales near-linearly, $t\propto m^{1.17}$
with either near-linear engine (LAMG$+$ and AC fit essentially the same exponent, $1.16$ and $1.18$), while
...
all size-independent, so the wall-clock reflects a single near-linear inner solve, ${\approx}m^{1.17}$
for both engines. The two engines are
...
the equilibrium solve (default LAMG$+$ engine) across a topology-diverse corpus of $20$ irregular
...
inner-engine setup count ($[87,311]$, one build per fine $\delta$-level)---a descriptive near-linear trend across real graphs of every family, not a
...
(default) and AC (interchangeable)---vs.\ the direct sparse factorization. LAMG$+$ and AC fit essentially the
...
engines---LAMG$+$ (default) and AC, both fitting $m^{1.17}$---vs.\ superlinear direct
factorization ($m^{1.48}$), the near-linear engines faster than the direct baseline across the range
...
Newton--multigrid inner reaches relative residual $10^{-9}$ in under two minutes on every instance; FISTA, given a $300$-second
