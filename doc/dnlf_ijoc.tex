...
toll design attains the marginal-cost optimum. The wall-clock scales near-linearly in the edge count while a
...
road networks, destination-bundled design cuts total system travel time by $7.7$--$13.1\%$ (at a calibrated
route-choice dispersion $\gamma$). Code is available at \url{https://github.com/orenlivne/dnlf}.
}
...
small perturbation, the total number of inner-engine setups (one build per level) is bounded independently of
graph size $m$; solving intermediate levels only loosely ($\varepsilon_{\mathrm{int}}$) is a faster variant that
...
$\R^n/\langle\mathbf1\rangle$ whenever $\sum_a d_a=0$, the graph is strongly connected, and $d$ is
...
(relative flow error $\sim10^{-9}$). This matches a scaled marginal-cost (Pigouvian) toll sweep, which bottoms out at $5.12\%$---indistinguishable
from the gradient result to within the equilibrium-solve tolerance, confirming near-stationarity. Solving every equilibrium only to a loose $10^{-3}$ tolerance reaches the \emph{same} optimum (TSTT within
...
every step (superlinearly, fill-dominated), capping it near ${\sim}10^2$ links; the question is whether the
...
moderate congestion (peak volume/capacity ${\approx}10$). Projected-gradient tolling takes $20$ accepted steps to
...
total including the cold solve. The two operations an SAB step performs by direct factorization are here both
...
two-to-three orders of magnitude beyond the SAB ceiling.
...
whole range. Direct is capped at $m\le4.5\times10^4$ (\emph{---}): the fine-continuation solve performs ${\sim}100$
...
disabled) is used for the correctness and design results (\crefrange{sec:results}{sec:design}); the loose-homotopy,
...
$\eta$ & inner AMG/PCG tolerance & $5\times10^{-2}$ & \texttt{eta}\\
--- & loose homotopy, adaptive polish & on & \texttt{tight\_last=false}\\
$\gamma$ & logit dispersion (target) & $0.1$; $2,5$ (roads) & \texttt{gamma}\\
...
Frank--Wolfe. (ii) \textbf{Accuracy on the most hub-dominated graphs.} On a handful of extreme hub graphs (dense web crawls)
