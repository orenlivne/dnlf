...
rectified equilibrium (\cref{alg:solve}). Because the number of $\delta$-levels is a fixed constant, the total number of inner-engine setups is bounded independently of graph size $m$.
...
On size-scaled irregular (poorly-separable) congested networks the equilibrium wall-clock scales near-linearly, $t\propto m^{1.11}$
with either near-linear engine (the default LAMG$+$ and AC fit the \emph{same} exponent), while
...
COLAMD ordering, the baseline we run) grows faster than the graph on a poorly-separable topology. NLF overtakes the direct one at $m\approx1.5\times10^{4}$ edges ($n\approx1{,}500$), beyond which it is faster
...
separation is the memory wall direct factorization hits and NLF does not---a genuine
...
all size-independent, so the wall-clock exponent reflects a single {near-linear inner solve---$m^{1.11}$
for both engines (the reason for the slight super-linearity in practice is lack of memory locality). The two engines are
...
$2.5\times10^4$ to $7\times10^5$ arcs.  Timing was serial and uncontended. The wall-clock fits
$t\propto m^{0.99}$ and the inner-engine setup count stays bounded across the entire corpus
...
Six of these corpus graphs, at accessible sizes below the direct-solver crossover ($n\le2.6\times10^4$), we also run head-to-head against the fair frozen-per-level direct baseline (\cref{tab:corpus}, timed with AC). The comparison is instance-dependent: the NLF solve is $1.5$--$1.6\times$ faster on the sparsest overlay/AS graphs (\texttt{p2p-Gnutella08}, \texttt{Oregon-1}), essentially tied on \texttt{as-caida}, and slower on the smallest (\texttt{as-735}) and the denser \texttt{p2p-Gnutella24}, where a good fill-reducing ordering still wins. Which solver wins is set by the graph's separator structure, not its edge count alone; NLF is competitive-to-faster on the actual application topologies, with the advantage concentrated on the sparsest, least-separable instances.
...
clean run on an Apple M5 Pro: the two near-linear engines---LAMG$+$ (default) and AC
(interchangeable)---vs.\ the fair frozen-per-level direct factorization. LAMG$+$ and AC fit the
...
workstation (random-graph fill-in), where both near-linear engines still run. AC's randomized
elimination makes it less predictable at the largest size (the $547.7$~s here rose to $856$~s on a second clean
run, vs.\ LAMG$+$'s stable $526$--$533$~s); LAMG$+$ has a cleaner near-linear complexity, and used by default.}
\label{tab:scaling}
...
engines---LAMG$+$ (default) and AC, both fitting $m^{1.11}$---vs.\ superlinear direct
...
Newton--multigrid inner reaches relative residual $10^{-9}$ in seconds; FISTA, given a $300$-second
...
AC, loose-intermediate continuation) to relative residual $10^{-9}$; FISTA (accelerated
