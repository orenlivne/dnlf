...
toll design matches the best scaled marginal-cost (Pigouvian) toll to within near-stationarity. The wall-clock scales near-linearly in the edge count while a
direct factorization scales superlinearly, so on large irregular congestion networks NLF is uniformly faster
with a widening margin and reaches sizes where a direct factorization is infeasible; the design inherits this
...
LAMG$+$~\citep{lamgplus} by default; AC~\citep{kyngsachdeva2016} is interchangeable (the two
fit the same $m^{1.17}$ scaling; \cref{tab:scaling}).
...
On size-scaled irregular (poorly-separable) congested networks the fine-continuation solve reaches a $10^{-9}$ equilibrium residual with a wall-clock that scales near-linearly, $t\propto m^{1.17}$, while
the same continuation with a direct sparse-factorization inner solve scales superlinearly, $t\propto m^{1.48}$
(\cref{tab:scaling}, \cref{fig:scaling}): each of the ${\sim}100$ inner solves is a fill-reducing sparse factorization (UMFPACK with
COLAMD ordering) whose fill-in grows faster than the graph on a poorly-separable topology. NLF is faster than the direct baseline across the whole tested range, with a widening margin---$1.7\times$ at $m=4\times10^4$ ($20$ vs.\ $33$~s)---and it reaches
$m=6.4\times10^5$ in ${\approx}424$~s, where a direct factorization is infeasible: on our workstation the random-graph fill-in exhausts RAM.
The direct baseline uses UMFPACK with COLAMD, an unsymmetric multifrontal solver, whereas $J$ is symmetric
positive semidefinite; a supernodal Cholesky under nested dissection (CHOLMOD$+$METIS) would lower the constant
and push the RAM ceiling out, shifting the crossover point. It would not, however, remove the asymptotic
separation: on poorly-separable topologies nested dissection has weak vertex separators, so Cholesky fill still
grows superlinearly, and the near-linear-vs.-superlinear gap is a property of the fill, not of the ordering.
...
clean run on an Apple M5 Pro; every solve reaches a $10^{-9}$ residual. The two near-linear engines---LAMG$+$
(default) and AC---vs.\ the direct sparse factorization. Both fit the
same exponent, $m^{1.17}$ and agree to within ${\approx}1.5\times$ per point; direct scales as $m^{1.48}$ and is slower across the
whole range. Direct is capped at $m\le4\times10^4$ (\emph{---}): the fine-continuation solve performs ${\sim}100$
inner solves, each a full factorization here, whose fill-in exhausts RAM at larger $m$.}
\label{tab:scaling}
\begin{tabular}{rrrrr}\toprule
$n$ & $m$ & LAMG$+$ & AC & direct\\\midrule
$1{,}000$  & $9{,}964$   & $3.68$   & $3.88$   & $4.20$\\
$2{,}000$  & $19{,}946$  & $10.43$  & $9.75$   & $12.67$\\
$4{,}000$  & $39{,}952$  & $19.71$  & $19.97$  & $32.88$\\
$8{,}000$  & $79{,}936$  & $46.10$  & $45.09$  & \emph{---}\\
$16{,}000$ & $159{,}954$ & $117.06$ & $180.62$ & \emph{---}\\
$32{,}000$ & $319{,}952$ & $281.88$ & $237.31$ & \emph{---}\\
$64{,}000$ & $639{,}950$ & $424.39$ & $449.99$ & \emph{---}\\\bottomrule
\end{tabular}
\end{table}
...
engines LAMG$+$ and AC both fit $m^{1.17}$, vs.\ superlinear direct
factorization ($m^{1.48}$), and are always faster across the range
...
pins all software versions (Julia, \texttt{Laplacians.jl} for approximate Cholesky, LAMG$+$, and
UMFPACK/COLAMD for the direct baseline). The synthetic instances (\cref{tab:scaling,tab:baseline,tab:mcdesign})
