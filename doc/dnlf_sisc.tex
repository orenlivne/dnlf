...
near-linear solver does \emph{not} win; the advantage, for the time being, is on the poorly-separable networks that model selfish
...
near-linear Laplacian solver wins. Our scaling study is therefore on irregular networks, and we are explicit that---for the time being---the
near-linear advantage is established only on these non-planar, poorly-separable topologies; the planar (road)
case is \emph{not} where the method wins, as a direct nested-dissection factorization is already near-optimal there. The most literal instance is selfish
...
better as $\gamma\to0$ would only matter for that separate deterministic model. (ii) \textbf{Corpus scaling.} The near-linear behaviour is confirmed both on the
controlled synthetic family ($m^{1.11}$) and across a $53$-graph topology-diverse SuiteSparse/SNAP corpus
($m^{0.99}$ descriptive fit, with the inner-engine setup count bounded in $[7,27]$; \cref{fig:corpus}); a
still-larger corpus (thousands of graphs) would tighten the estimate further. (iii) \textbf{One hierarchy across design steps.} Reusing a single frozen hierarchy across all
