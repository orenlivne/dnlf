...
\usepackage{amsmath,amssymb,booktabs,graphicx,enumitem,algpseudocode,algorithm}
\newcommand{\R}{\mathbb{R}}
\newcommand{\Om}{\mathcal{O}(m)}
\hypersetup{colorlinks=true,allcolors=blue!55!black}

\title{A Near-Linear-Time Solver for Bilevel Congestion Network Design
via Directed Nonlinear Laplacian Flow\thanks{Submitted to the editors \today.}}
\author{Oren E. Livne\thanks{Pine Birch Analytics, Churchville, PA
(\texttt{oren.livne@gmail.com}).}}
\headers{Near-linear bilevel congestion network design}{O. E. Livne}
...
per equilibrium solve is bounded independently of the graph size, and---using loose tolerances at intermediate
continuation levels and a single accurate solve at the end---the wall-clock scales near-linearly in the edge
count while a direct factorization scales superlinearly, so on large \emph{irregular} congestion networks
(scale-free communication and overlay topologies) the near-linear solver overtakes the direct factorization at a
crossover; the design optimization inherits this near-linear per-step cost. We are explicit that road networks,
being near-planar, are the regime where a direct nested-dissection factorization is already near-optimal and the
near-linear solver does \emph{not} win; the advantage is on the poorly-separable networks that model selfish
routing in communication systems. We further extend the framework to the \emph{multicommodity} logit stochastic
...
matrices, destination-bundled design cuts total system travel time by $8$--$13\%$ with a bounded,
size-independent inner-iteration count. Code is available at \url{https://github.com/orenlivne/dnlf}.
\end{abstract}

\begin{keywords}
network design, traffic assignment, user equilibrium, bilevel optimization, nonlinear Laplacian flow,
algebraic multigrid, adjoint sensitivity, congestion pricing
\end{keywords}

\begin{AMS}
90B20, 90C35, 65F08, 65K10, 90C33, 49M15
\end{AMS}
...
It is nonconvex and nondifferentiable, and \emph{NP}-hard even with affine latencies~\cite{gairing2017}; second-best
tolling is \emph{NP}-hard as well~\cite{hoefer2008}. Practical methods therefore seek a local (KKT) stationary
point, and essentially all of them---sensitivity-analysis-based (SAB) descent~\cite{tobinfriesz1988,jp2007,chiou2005},
mathematical programs with equilibrium constraints, and metaheuristics---share one structure: an \emph{outer}
loop in which each iteration solves the lower-level equilibrium and then a sensitivity (equilibrium derivative).

\paragraph{The cost, and where it lives.}
The design gradient itself is cheap in the number of design variables: since Tobin and
Friesz~\cite{tobinfriesz1988}, $\nabla_\tau F$ is obtained by factorizing the equilibrium Jacobian \emph{once}
and back-substituting one right-hand side per design variable, so its cost is independent of $k$ (an adjoint /
implicit-function computation~\cite{jp2007,amos2017,blondel2022}). The bottleneck is that this Jacobian
factorization---and the lower-level solve that precedes it---is a \emph{direct} sparse factorization whose cost
is superlinear (cubic in the active-path/subnetwork size) or a slowly converging Frank--Wolfe assignment. This
is why exact design methods in the literature stall near Sioux Falls scale ($76$ links) while origin-based
forward assignment routinely reaches $4\times10^4$--$7.5\times10^4$ links~\cite{bargera2002,dial2006}. The gap
between forward and design scalability is one to two orders of magnitude, and it is a \emph{linear-algebra} gap.
...
\item \textbf{A Laplacian structure for the directed equilibrium (\cref{sec:formulation}).} We write the
separable directed UE as a source-form nonlinear Laplacian flow $B\rho(B^\top\phi-\tau)=d$ with a one-sided
(rectified) edge law, and show that the Newton Jacobian is a \emph{symmetric weighted graph Laplacian on the
active subgraph} (\cref{prop:laplacian}). Orientation and one-sidedness do not break the Laplacian; they only
determine which arcs are active. Hence the entire near-linear Laplacian toolbox applies.
...
polish. The number of setups is bounded \emph{independently of the graph size}.
...
\end{enumerate}
\paragraph{Scope: where the advantage lives.}
The lower-level equilibrium \cref{eq:nlf} is the Wardrop (user) equilibrium of a nonlinear \emph{congestion
game}~\cite{rosenthal1973,roughgarden2002}, and the design problem \cref{eq:bilevel} is the optimal-tolling /
Stackelberg problem of reducing the price of anarchy~\cite{swamy2012,korilis1997}. This model describes selfish
routing not only in road traffic but in \emph{communication and overlay networks}---multipath and elastic-traffic
routing games in the Internet, data-center fabrics, and peer-to-peer overlays~\cite{orda1993,larroca2013}. The
distinction matters for scalability: road networks are near-planar, where a direct nested-dissection
factorization of the equilibrium Jacobian is near-optimal and a near-linear Laplacian solver has nothing to
beat. Communication and overlay topologies, by contrast, are large and \emph{poorly separable} (scale-free
degree, no planar embedding), so a direct factorization fills in superlinearly---exactly the regime in which a
near-linear Laplacian solver wins. Our scaling study is therefore on irregular networks, and we are explicit that
the planar (road) case is \emph{not} where the method has an advantage. The most literal instance is selfish
\emph{overlay} routing, where end hosts genuinely choose paths~\cite{qiu2003}, on Internet AS- and router-level
graphs that are provably scale-free~\cite{faloutsos1999}. We do not overclaim: production traffic engineering
optimizes operator-set link weights or bandwidth allocations by linear programming and local
search~\cite{fortz2000}, not by pricing a selfish equilibrium, and packet-level congestion control is a distinct
(utility-maximization) equilibrium. Our contribution is a scalable solver for the well-established
equilibrium/design \emph{model} on standard large irregular topologies, complementing rather than replacing
operator tools.

The construction reuses the nonlinear Laplacian flow (NLF) solver~\cite{nlf} and its near-linear inner engines,
approximate Cholesky~\cite{kyngsachdeva2016} and LAMG$+$~\cite{lamgplus}. We treat both the single-commodity
...
to a handful of iterations. Remaining scope (\cref{sec:conclusion}): an active-set deflation that would make \emph{one}
hierarchy valid across all design steps (reducing setups from $\mathcal{O}(1)$-per-solve to $\mathcal{O}(1)$
total) is left open.
...
\qquad \rho'_a=\tfrac{1}{t'_a(f_a)}>0 \text{ on active arcs},\ \rho'_a=0 \text{ else}.
\]
\end{proposition}
\begin{proof}
...
arcs are active (a routing boundary), not through the type of the linear operator. Every linearized solve is a
graph-Laplacian solve, so a near-linear Laplacian engine applies. This is complete for the single-commodity
...
framework~\cite{nlf}. Two features of the directed problem require care.
...
free-flow threshold $t^0_a$, so the operator that a multigrid hierarchy is built for is \emph{stale} a few Newton
steps later. A hierarchy frozen across the whole solve stalls short of the equilibrium.
...
\emph{everywhere} (here $\sigma$ is the logistic function), so $J$ is a full, uniformly well-conditioned
Laplacian with no dead zone. We continue $\delta$ geometrically from $\delta_0=\tfrac12\bar t^0$ down to a small
value, building one AMG hierarchy at the start of each $\delta$-level and freezing it \emph{within} the level;
the smoothed conductances vary slowly within a level, so the frozen hierarchy is valid there. A final
\emph{polish} with the exact law \cref{eq:rho}, warm-started from the smoothed solution, recovers the exact
rectified equilibrium (\cref{alg:solve}). Because the number of $\delta$-levels is a fixed constant, the number
of AMG setups per solve is bounded independently of the graph size $m$.

The method restated here is the chord-Newton flow solver; we give a self-contained convergence guarantee for the
...
toward $\delta\to0$, and the polish is a warm-started solve of the exact (piecewise-smooth, still convex) energy.

\begin{algorithm}
\caption{Near-linear directed-UE solve (cold start)}\label{alg:solve}
\begin{algorithmic}[1]
...
\For{$\ell=0,\dots,L$} \Comment{smoothing homotopy: one hierarchy build per level}
  ...
  $B\rho_{\delta_\ell}(B^\top\phi-\tau)=d$ to tolerance, hierarchy \textbf{frozen} within the level
\EndFor
\State \textbf{polish:} chord-Newton on the exact law $B\rho(B^\top\phi-\tau)=d$, warm-started from $\phi$,
...
\end{proof}
This recovers the classical $k$-independence of equilibrium sensitivity~\cite{tobinfriesz1988,jp2007} but at
...
We use the Sioux Falls benchmark ($n=24$, $m=76$) and size-scaled grid networks. The inner engine is approximate
Cholesky~\cite{kyngsachdeva2016} by default; LAMG$+$~\cite{lamgplus} is interchangeable.

\paragraph{Correctness of the equilibrium.}
...
destination pairs, i.e.\ to Frank--Wolfe's own accuracy. The number of approximate-Cholesky setups is $10$--$25$
per solve, set by the fixed number of $\delta$-levels and independent of graph size.

\paragraph{Correctness of the adjoint.}
...
quality. The reduction is modest because a single origin--destination pair admits little rerouting; the
multicommodity instances of \cref{sec:mc} exercise the design problem more fully.

\paragraph{Scaling on irregular networks.}
...
(poorly-separable) congested networks the equilibrium wall-clock then scales near-linearly, $t\propto m^{1.23}$,
while the same continuation with a direct sparse-factorization inner solve scales superlinearly, $t\propto
m^{1.38}$ (\cref{tab:scaling}, \cref{fig:scaling}): the fill-in of a fill-reducing sparse direct factorization
(UMFPACK with COLAMD ordering, the baseline we run) grows faster than the graph on a poorly-separable topology. The near-linear solver overtakes the direct one at $m\approx
2\times10^{4}$ edges ($n\approx2\times10^3$), beyond which it is faster with a slowly widening margin---about
$1.6\times$ at $m=1.6\times10^5$ ($110$ vs.\ $176$~s)---and it reaches $m=6.4\times10^5$ in $909$~s, where the
direct factorization is infeasible: on our workstation it already exhausts memory at $m=3.2\times10^5$, the
random-graph fill-in exceeding RAM (\cref{tab:scaling}). Below the crossover the constant favors the direct
solver. The wall-clock gap between the two exponents is real but modest at feasible sizes ($\sim\!1.6\times$);
the sharper practical separation is that direct factorization hits a memory wall the near-linear solver does
not. We claim a genuine near-linear-versus-superlinear crossover on this graph class, not a large-factor
speedup.
The near-linear exponent is inherited from the inner solve, not the outer iteration: across these sizes the
number of chord-Newton steps ($\approx\!625$) and AMG setups ($\approx\!14$) are both constant
(size-independent), so the $m^{1.23}$ wall-clock reflects the empirical scaling of a single approximate-Cholesky
solve (near-linear, $m\,\mathrm{polylog}\,m$ in theory, $\approx m^{1.2}$ in practice on these graphs) rather
than any growth in the iteration count. Loose intermediate tolerances realize this: solving every level tightly
instead multiplies the Newton-step count and steepens the wall-clock, without changing the final solution
(\cref{sec:results}).

We corroborate this on \emph{real} irregular networks from the SuiteSparse/SNAP collection---Internet
autonomous-system graphs and peer-to-peer overlays, the topologies of the selfish-routing application
(\cref{tab:corpus}). The AMG setup count is size-independent ($14$--$20$ builds) across the corpus, confirming
...
edge count alone. The controlled synthetic family (\cref{tab:scaling}) isolates the asymptotic crossover
cleanly; the real corpus shows the near-linear solver is already competitive-to-faster on the actual
application topologies, with the advantage concentrated on the sparsest, least-separable instances.
...
frozen-per-level direct baseline; AMG \emph{setups} stay size-independent ($14$--$20$ across the corpus).
...
\begin{table}[t]\centering
\caption{Equilibrium solve time (s) on size-scaled irregular congested networks (random degree-5 graphs):
near-linear (approximate Cholesky, loose-intermediate continuation) vs.\ the fair frozen-per-level direct
factorization. Ratio $=t_{\mathrm{approxChol}}/t_{\mathrm{direct}}$ (bold${}<1$: near-linear faster).
Least-squares fits over the feasible rows: $t_{\mathrm{approxChol}}\propto m^{1.23}$, $t_{\mathrm{direct}}\propto
m^{1.38}$, empirical crossover $m\approx2\times10^4$ (fitted lines meet at ${\approx}7\times10^3$). \emph{OOM}:
direct factorization exhausts memory at
$m\ge3.2\times10^5$ on our workstation (random-graph fill-in), where the near-linear solver still runs. Single
run; per-cell wall-clock varies run to run (see \cref{tab:corpus}).}
\label{tab:scaling}
\begin{tabular}{rrrrr}\toprule
$n$ & $m$ & approx.\ Cholesky & direct & ratio\\\midrule
$1{,}000$  & $9{,}964$   & $4.62$   & $4.89$   & $\mathbf{0.95}$\\
$2{,}000$  & $19{,}946$  & $11.18$  & $11.05$  & $1.01$\\
$4{,}000$  & $39{,}952$  & $26.06$  & $29.19$  & $\mathbf{0.89}$\\
$8{,}000$  & $79{,}936$  & $54.19$  & $120.51$ & $\mathbf{0.45}$\\
$16{,}000$ & $159{,}954$ & $109.74$ & $175.85$ & $\mathbf{0.62}$\\
$32{,}000$ & $319{,}952$ & $298.96$ & \emph{OOM} & ---\\
$64{,}000$ & $639{,}950$ & $909.38$ & \emph{OOM} & ---\\\bottomrule
\end{tabular}
\end{table}
...
\caption{Equilibrium solve time vs.\ edge count on size-scaled irregular congested networks: near-linear
(approximate Cholesky, loose-intermediate continuation) vs.\ superlinear (direct factorization), crossing near
$m\approx2\times10^4$; least-squares fits $m^{1.23}$ and $m^{1.38}$.}\label{fig:scaling}\end{figure}

\paragraph{Comparison with differentiable-bilevel (first-order) solvers.}
The learning-based approach to Stackelberg congestion design~\cite{li2022,goyal2023} treats the lower-level
...
All code, unit tests, and reproduction drivers are available at \url{https://github.com/orenlivne/dnlf}.
\texttt{examples/reproduce.jl} regenerates the Sioux Falls correctness, adjoint, and design numbers of this
section with timing; \texttt{scripts/scaling\_synthetic.jl} regenerates the size-scaled crossover
(\cref{tab:scaling}, \cref{fig:scaling}) and its fitted exponents; \texttt{scripts/scaling.jl} regenerates the
real-corpus table (\cref{tab:corpus}) over the public SuiteSparse/SNAP graph collection; both compare against
the fair frozen-per-level direct baseline. \texttt{scripts/baseline\_firstorder.jl} regenerates the
first-order head-to-head (\cref{tab:baseline}); \texttt{scripts/scaling\_mc.jl} regenerates the multicommodity
iteration table (\cref{tab:mc}), \texttt{scripts/design\_study\_mc.jl} the multi-instance design study
(\cref{tab:mcdesign}), \texttt{scripts/design\_study\_tntp.jl} the real-network validation on the published
Sioux Falls and Anaheim OD matrices (\cref{tab:mcreal}), and \texttt{examples/reproduce\_mc.jl} a single
multicommodity design instance (\cref{sec:mc}); and \texttt{test/runtests.jl} verifies every algorithm
component, including \cref{prop:laplacian,prop:conv,prop:adjoint,prop:mc} and the TNTP OD-matrix loader. The
test networks are the TNTP Sioux Falls and Anaheim instances with their published OD matrices (shipped with the
code) and public Internet AS / P2P-overlay graphs (e.g.\ \texttt{as-caida}, \texttt{Oregon-1},
\texttt{p2p-Gnutella}) from the SuiteSparse/SNAP collection.
...
the framework to the multicommodity \emph{logit stochastic user equilibrium} (SUE)~\cite{fisk1980,daganzo1977},
...
\begin{proposition}[the multicommodity Newton system is a coupled block Laplacian]\label{prop:mc}
At a logit equilibrium the reduced Newton Jacobian in the stacked potentials $\Phi=[\phi^1;\dots;\phi^K]$ is
\begin{equation}\label{eq:mcjac}
  J=\underbrace{\tfrac1\gamma\,\mathrm{blkdiag}_k\!\bigl(B\,\mathrm{diag}(f^k)\,B^\top\bigr)}_{K\ \text{per-commodity Laplacians}}
   -\tfrac1{\gamma^2}\sum_a c_a\,(f_a f_a^\top)\otimes b_a b_a^\top,\qquad
   c_a=\frac{t'_a}{1+t'_a f_a/\gamma},
\end{equation}
where $b_a=Be_a$ and $f_a=(f^1_a,\dots,f^K_a)^\top$. Each diagonal block is a symmetric weighted graph Laplacian;
...
$f_a/(\gamma+t'_af_a)\to1/t'_a$ as $\gamma\to0$.
\end{proof}
...
block diagonal---one near-linear approximate-Cholesky solve per commodity. The rank-one commodity coupling is
exactly Brandt's \emph{distribution matrix}~\cite{brandt1984}: in the mean/deviation basis over commodities it
...
dispersion~\cite{fisk1980,daganzo1977}, a complete behavioural model in its own right, so we solve the logit SUE
at that $\gamma$ and do not pursue $\gamma\to0$. We nonetheless verified that a coupling-aware distribution-matrix
preconditioner realized in flow space (the per-arc coupling inverted exactly by Sherman--Morrison, with an
aggregate-congestion Laplacian on the mean commodity mode, as \cite{brandt1984} prescribes) converges
$\gamma$-boundedly where the reduced-potential-space version does not, but it does not improve on the block
diagonal in the operating regime; the block diagonal is therefore retained. Fixed $\gamma$ is thus both the
...
central finite differences to under $10^{-3}$ (verified in \texttt{test/runtests.jl}), and on a multicommodity
...
$K=38$)---destination-bundled design reduces the real system travel time by $8$--$13\%$ (\cref{tab:mcreal}). Both
networks are over-saturated at their full published demand (peak volume/capacity above $2.5$); the inner
iteration count stays bounded and, consistent with the near-linear claim, does \emph{not} grow with size---Anaheim
carries $27\times$ the degrees of freedom of Sioux Falls yet needs \emph{fewer} inner iterations, its larger arc
set better conditioning the block preconditioner. A budget-constrained (sparse-toll) design is a natural extension.
...
size $n$ ($K=8$, spread). Single-digit to low double digit, $\log K$ growth, $n$-independent; regenerated by
\texttt{scripts/scaling\_mc.jl}.}
\label{tab:mc}
...
maximum). The inner count stays single digit throughout. Regenerated by \texttt{scripts/design\_study\_mc.jl}.}
\label{tab:mcdesign}
...
inner conjugate-gradient count at the final equilibrium solve is bounded and does not grow with size---Anaheim
has $27\times$ the degrees of freedom yet needs fewer iterations. Regenerated by
\texttt{scripts/design\_study\_tntp.jl}.}
\label{tab:mcreal}
...
CNDP and second-best tolling are bilevel/MPEC problems, nonconvex and \emph{NP}-hard~\cite{gairing2017,hoefer2008};
surveys~\cite{farahani2013} catalog SAB descent, MPEC, and metaheuristic methods. Equilibrium sensitivity
originates with Tobin and Friesz~\cite{tobinfriesz1988} and was sharpened by Qiu and Magnanti and by Patriksson
and co-authors~\cite{jp2007}, who observed that the sensitivity problem is itself a linearized equilibrium solve.
...
congestion game~\cite{roughgarden2002,swamy2012,korilis1997}, and in communication networks as competitive /
selfish routing~\cite{orda1993,larroca2013}; that literature focuses on existence, bounds, and mechanism design
rather than on a scalable solver for the induced equilibrium, which is our concern. Recent differentiable-bilevel
methods
\cite{li2022,goyal2023} break the cold-resolve-per-step pattern with warm-started, truncated first-order inner
...
Nearly-linear-time solvers for symmetric diagonally dominant systems~\cite{kyngsachdeva2016,lamgplus} underpin
web-scale spectral graph computation but have not, to our knowledge, been used as the inner solve of a bilevel
traffic-design loop. The nonlinear Laplacian flow framework~\cite{nlf} unifies max-flow, congestion, and
...
We showed that the directed traffic user equilibrium with separable costs is a nonlinear Laplacian flow whose
Jacobian is a symmetric Laplacian on the active subgraph, and that a smoothing homotopy makes its solution---and
the adjoint design gradient---a short sequence of near-linear Laplacian solves, with a graph-size-independent
number of multigrid setups and near-linear wall-clock. Bilevel toll design then inherits a near-linear per-step
cost, in place of the cubic factorization of sensitivity-analysis methods.

We are explicit about what this paper does \emph{not} yet establish. (i) \textbf{Multicommodity scope.}
\Cref{sec:mc} solves and designs the multicommodity \emph{logit} (stochastic) user equilibrium at a fixed,
calibrated dispersion $\gamma$, validated on both synthetic instances and the two canonical TNTP road networks
(Sioux Falls, Anaheim) with their published OD matrices (\cref{tab:mcreal}). We do \emph{not} pursue the
$\gamma\to0$ \emph{deterministic} multicommodity limit: that is a distinct model, with non-unique per-commodity
flows, for which Frank--Wolfe and related methods are already effective, and reaching it is outside the scope of
a fixed-$\gamma$ SUE solver rather than a deficiency of it. The coupling-aware distribution-matrix preconditioner
we investigated (\cref{sec:mc}) converges $\gamma$-boundedly in its flow-space realization, confirming Brandt's
construction, but does not improve on the block diagonal in the operating regime; a preconditioner strictly
better as $\gamma\to0$ would only matter for that separate deterministic model. (ii) \textbf{Corpus scaling.} The near-linear claim is shown on one size-scaled
family; a study across a large graph corpus (as for related near-linear solvers) is needed to state the exponent
with confidence. (iii) \textbf{One hierarchy across design steps.} Reusing a single frozen hierarchy across all
design steps saves only a modest factor, because each exact solve intrinsically rebuilds once per $\delta$-level:
the exploding condition number reflects a near-null-space mode appearing when an active-set boundary is crossed.
Capturing that mode by deflation or recombination---treating a global active-set mode on coarse levels---would
make one hierarchy valid across active-set changes and reduce the setups to $\mathcal{O}(1)$ \emph{total}; this
is the main open algorithmic problem. (iv) \textbf{Planarity.} Road networks are near-planar, where a near-linear
solver ties nested-dissection direct methods; the decisive advantage of the near-linear inner solve is on
memory at very large scale and on non-planar (e.g.\ communication or logistics) design instances. (v)
\textbf{Saturating laws and folds.} A capacity-saturating (min-delay) law reintroduces a genuine turning-point
fold, where the continuation/deflation machinery of~\cite{nlf} applies; the design problem through the fold is
future work.

\section*{Acknowledgments}
This work builds on the NLF and LAMG$+$ solvers; the author thanks the developers of Laplacians.jl for the
approximate-Cholesky engine.

\begin{thebibliography}{99}
\bibitem{tobinfriesz1988} R.~L.~Tobin and T.~L.~Friesz, \emph{Sensitivity analysis for equilibrium network flow},
Transportation Science, 22 (1988), pp.~242--250.
\bibitem{jp2007} M.~Josefsson and M.~Patriksson, \emph{Sensitivity analysis of separable traffic equilibria with
application to bilevel optimization in network design}, Transportation Research Part B, 41 (2007), pp.~4--31.
\bibitem{chiou2005} S.-W.~Chiou, \emph{Bilevel programming for the continuous transport network design problem},
Transportation Research Part B, 39 (2005), pp.~361--383.
\bibitem{gairing2017} M.~Gairing, T.~Harks, and M.~Klimm, \emph{Complexity and approximation of the continuous
network design problem}, SIAM Journal on Optimization, 27 (2017), pp.~1554--1582.
\bibitem{hoefer2008} M.~Hoefer, L.~Olbrich, and A.~Skopalik, \emph{Taxing subnetworks}, in Proc.\ WINE, 2008,
pp.~286--294.
\bibitem{farahani2013} R.~Z.~Farahani, E.~Miandoabchi, W.~Y.~Szeto, and H.~Rashidi, \emph{A review of urban
...
\bibitem{dial2006} R.~B.~Dial, \emph{A path-based user-equilibrium traffic assignment algorithm that obviates
path storage and enumeration}, Transportation Research Part B, 40 (2006), pp.~917--936.
\bibitem{li2022} P.~Li et al., \emph{Differentiable bilevel programming for Stackelberg congestion games},
arXiv:2209.07618, 2022.
\bibitem{goyal2023} A.~Goyal and A.~Lamperski, \emph{An algorithm for bilevel optimization with traffic
equilibrium constraints}, arXiv:2306.14235, 2023.
\bibitem{kyngsachdeva2016} R.~Kyng and S.~Sachdeva, \emph{Approximate Gaussian elimination for Laplacians---fast,
sparse, and simple}, in Proc.\ FOCS, 2016, pp.~573--582.
\bibitem{lamgplus} O.~E.~Livne, \emph{LAMG$+$: a robust lean algebraic multigrid solver for graph Laplacians},
arXiv:2606.24791; accepted, SIAM Journal on Scientific Computing.
\bibitem{nlf} O.~E.~Livne, \emph{NLF: a resistor-network framework and linear-time solver for convex network-flow
equilibria}, SIAM Journal on Scientific Computing (under review), 2026.
\bibitem{fisk1980} C.~Fisk, \emph{Some developments in equilibrium traffic assignment}, Transportation Research
Part B, 14 (1980), pp.~243--255.
\bibitem{daganzo1977} C.~F.~Daganzo and Y.~Sheffi, \emph{On stochastic models of traffic assignment},
Transportation Science, 11 (1977), pp.~253--274.
\bibitem{brandt1984} A.~Brandt, \emph{Multigrid techniques: 1984 guide with applications to fluid dynamics},
GMD-Studien Nr.~85, Gesellschaft f\"ur Mathematik und Datenverarbeitung, 1984 (distributive/distribution-matrix
relaxation, \S3.4).
\bibitem{amos2017} B.~Amos and J.~Z.~Kolter, \emph{OptNet: differentiable optimization as a layer in neural
networks}, in Proc.\ ICML, 2017, pp.~136--145.
\bibitem{blondel2022} M.~Blondel et al., \emph{Efficient and modular implicit differentiation}, in Proc.\
NeurIPS, 2022.
\bibitem{rosenthal1973} R.~W.~Rosenthal, \emph{A class of games possessing pure-strategy Nash equilibria},
International Journal of Game Theory, 2 (1973), pp.~65--67.
\bibitem{roughgarden2002} T.~Roughgarden and \'E.~Tardos, \emph{How bad is selfish routing?}, Journal of the
ACM, 49 (2002), pp.~236--259.
\bibitem{swamy2012} C.~Swamy, \emph{The effectiveness of Stackelberg strategies and tolls for network congestion
games}, ACM Transactions on Algorithms, 8 (2012), art.~36.
\bibitem{korilis1997} Y.~A.~Korilis, A.~A.~Lazar, and A.~Orda, \emph{Achieving network optima using Stackelberg
routing strategies}, IEEE/ACM Transactions on Networking, 5 (1997), pp.~161--173.
\bibitem{orda1993} A.~Orda, R.~Rom, and N.~Shimkin, \emph{Competitive routing in multiuser communication
networks}, IEEE/ACM Transactions on Networking, 1 (1993), pp.~510--521.
\bibitem{larroca2013} F.~Larroca and J.-L.~Rougier, \emph{Minimum-delay load-balancing through non-parametric
regression and traffic-engineering}, Computer Networks, 57 (2013), pp.~2716--2728.
\bibitem{qiu2003} L.~Qiu, Y.~R.~Yang, Y.~Zhang, and S.~Shenker, \emph{On selfish routing in Internet-like
environments}, in Proc.\ ACM SIGCOMM, 2003, pp.~151--162.
\bibitem{faloutsos1999} M.~Faloutsos, P.~Faloutsos, and C.~Faloutsos, \emph{On power-law relationships of the
Internet topology}, in Proc.\ ACM SIGCOMM, 1999, pp.~251--262.
\bibitem{fortz2000} B.~Fortz and M.~Thorup, \emph{Internet traffic engineering by optimizing OSPF weights}, in
Proc.\ IEEE INFOCOM, 2000, pp.~519--528.
\bibitem{cole2003} R.~Cole, Y.~Dodis, and T.~Roughgarden, \emph{Pricing network edges for heterogeneous selfish
users}, in Proc.\ ACM STOC, 2003, pp.~521--530.
\bibitem{leskovec2007} J.~Leskovec, J.~Kleinberg, and C.~Faloutsos, \emph{Graph evolution: densification and
