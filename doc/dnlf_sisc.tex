...
graph-Laplacian solve, so a near-linear Laplacian engine applies. The multicommodity generalization keeps the
same structure: with $f_a=\sum_k f_a^k$ and a cost of the total flow, the per-arc block Hessian is
$t'_a\,\mathbf{1}\mathbf{1}^\top$, symmetric, so the block system is again a Laplacian; we return to this in
\cref{sec:conclusion}.
...
m^{1.38}$ (\cref{tab:scaling}, \cref{fig:scaling}): the fill-in of a nested-dissection factorization grows faster
than the graph on a poorly-separable topology. The near-linear solver overtakes the direct one at $m\approx
...
good nested-dissection ordering still wins. Which solver wins is set by the graph's separator structure, not its
