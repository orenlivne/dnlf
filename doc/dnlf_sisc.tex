...
It is nonconvex and nondifferentiable, and \emph{NP}-hard even with affine latencies~\citep{gairing2017}; second-best
...
implicit-function computation~\citep{jp2007,amos2017,blondel2022}). The bottleneck is that this Jacobian
...
forward assignment routinely reaches $4\times10^4$--$7.5\times10^4$ links~\citep{bargera2002,dial2006}. The gap
...
graphs that are provably scale-free~\citep{faloutsos1999}. We do not overclaim: production traffic engineering
...
The construction reuses the nonlinear Laplacian flow (NLF) solver~\citep{nlf} and its near-linear inner engines,
LAMG$+$~\citep{lamgplus} (the default) and approximate Cholesky~\citep{kyngsachdeva2016} (interchangeable). We treat both the single-commodity
...
crossover the constant favors the direct solver. The wall-clock gap is real but modest at feasible sizes; the
sharper practical separation is that direct factorization hits a memory wall the near-linear solver does not. We
claim a genuine near-linear-versus-superlinear crossover on this graph class, not a large-factor speedup.
The near-linear exponent is inherited from the inner solve, not the outer iteration: across these sizes the
number of chord-Newton steps ($\approx\!625$) and inner-engine setups ($\approx\!14$) are both constant
(size-independent), and the per-step Jacobian assembly is itself linear ($t\propto m^{1.02}$), so the wall-clock
exponent reflects the empirical scaling of a single \emph{near-linear inner solve}---$m^{1.11}$ for both LAMG$+$
and approximate Cholesky ($m\,\mathrm{polylog}\,m$ in theory, the finite-range exponent inflated above $1$ by
memory locality)---rather than any growth in the iteration count. The two engines are interchangeable, tracking
the same $m^{1.11}$ curve within a few percent at every size; LAMG$+$ is the default for its cleaner theoretical
near-linear (multigrid $O(m)$) guarantee. It is also the steadier of the two at scale: approximate Cholesky's
randomized elimination gave $m=6.4\times10^5$ times of $547.7$~s (\cref{tab:scaling}) and $856$~s on two clean
runs, against LAMG$+$'s stable $526$--$533$~s. Loose intermediate tolerances realize this: solving every level tightly
...
(\cref{tab:corpus}; timed with approximate Cholesky, the interchangeable engine, at these sub-crossover sizes). The AMG setup count is size-independent ($14$--$20$ builds) across the corpus, confirming
...
word-association, and web networks---spanning $2.5\times10^4$ to $7\times10^5$ arcs ($1.5$ decades of edge count).
The wall-clock fits $t\propto m^{0.99}$, and the inner-engine setup count stays in $[7,27]$ across the
\emph{entire} corpus (\cref{fig:corpus}): near-linear and size-optimal on real graphs of every family, consistent
with the $m^{1.11}$ of the controlled synthetic family (\cref{tab:scaling}). The corpus fit is flatter than the
synthetic one because it mixes topologies of differing separator structure at each size rather than scaling a
single family; both are near-linear.
...
$53$ irregular SuiteSparse/SNAP graphs (autonomous-system, peer-to-peer, collaboration, email, signed-social,
citation, word-association, web). Wall-clock fits $t\propto m^{0.99}$ over $1.5$ decades, with a
size-independent inner-engine setup count ($[7,27]$)---near-linear on real graphs of every family. Serial,
uncontended timing on an Apple M5 Pro.}\label{fig:corpus}\end{figure}
...
macOS~26.5 and Julia~1.12. The test networks are the TNTP Sioux Falls and Anaheim instances with their published
...
aggregate-congestion Laplacian on the mean commodity mode, as \citet{brandt1984} prescribes) converges
...
inner iteration count stays bounded and does not grow with size (consistent with the size-independence
...
better as $\gamma\to0$ would only matter for that separate deterministic model. (ii) \textbf{Corpus scaling.} The near-linear exponent is confirmed both on the
controlled synthetic family ($m^{1.11}$) and across a $53$-graph topology-diverse SuiteSparse/SNAP corpus
($m^{0.99}$ with size-independent setups; \cref{fig:corpus}); a still-larger corpus (thousands of graphs) would
tighten the exponent further. (iii) \textbf{One hierarchy across design steps.} Reusing a single frozen hierarchy across all
...
fold, where the continuation/deflation machinery of~\citet{nlf} applies; the design problem through the fold is
future work.

\ACKNOWLEDGMENT{This work builds on the NLF and LAMG$+$ solvers; the author thanks the developers of the
Laplacians.jl package for the approximate-Cholesky engine.}
