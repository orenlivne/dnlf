...
routing in communication systems. Code is available at \url{https://github.com/orenlivne/dnlf}.
\end{abstract}
...
approximate Cholesky~\cite{kyngsachdeva2016} and LAMG$+$~\cite{lamgplus}. We are explicit about remaining scope
(\cref{sec:conclusion}): our experiments are single-commodity; multicommodity OD and a full corpus study are the
natural next steps, and an active-set deflation that would make \emph{one} hierarchy valid across all design
steps (reducing setups from $\mathcal{O}(1)$-per-solve to $\mathcal{O}(1)$ total) is left open.
...
equilibrium treated here (a general balanced demand, possibly multi-source/multi-sink). The multicommodity case
($f_a=\sum_k f_a^k$ with a cost of the total flow) preserves symmetry---the per-arc block coupling is
$t'_a\,\mathbf{1}\mathbf{1}^\top$---but the coupled block system is Laplacian-plus-low-rank per arc rather than a
plain Laplacian, and whether a near-linear engine handles it efficiently is open; we therefore scope the
near-linear claim to the single-commodity case and treat multicommodity as future work (\cref{sec:conclusion}).
...
quality. The reduction is modest because a single origin--destination pair admits little rerouting; larger,
multicommodity instances (\cref{sec:conclusion}) exercise the design problem more fully.
...
first-order head-to-head (\cref{tab:baseline}), and \texttt{test/runtests.jl} verifies every algorithm
component, including \cref{prop:laplacian,prop:conv,prop:adjoint}. The test networks are the TNTP Sioux Falls
...
We are explicit about what this paper does \emph{not} yet establish. (i) \textbf{Multicommodity OD.} Our
experiments are single-commodity; \cref{prop:laplacian} extends to the full OD table with a block Laplacian, and
implementing the destination-bundled multicommodity solve is the immediate next step for realistic
congestion-pricing instances. (ii) \textbf{Corpus scaling.} The near-linear claim is shown on one size-scaled
