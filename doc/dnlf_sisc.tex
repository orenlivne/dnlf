...
solver overtakes the direct one at $m\approx2\times10^{4}$ edges ($n\approx2\times10^3$), beyond which it is faster
with a widening margin---about $1.6\times$ at $m=1.6\times10^5$ ($99$ vs.\ $161$~s)---and it reaches
$m=6.4\times10^5$ in ${\approx}530$~s, where the direct factorization is infeasible: on our workstation it already
...
the same $m^{1.11}$ curve within a few percent at every size; LAMG$+$ is the default for its \emph{reproducible}
multigrid scaling---approximate Cholesky's randomized elimination showed larger run-to-run variance at the largest
size ($m=6.4\times10^5$: $548$--$856$~s across our clean runs, against LAMG$+$'s stable $526$--$533$~s)---and for
its cleaner theoretical near-linear guarantee. Loose intermediate tolerances realize this: solving every level tightly
...
elimination gives it larger run-to-run variance at the largest size ($548$--$856$~s across runs, vs.\ LAMG$+$'s
stable ${\approx}530$~s)---hence LAMG$+$ is the default.}
\label{tab:scaling}
...
congestion game~\citep{roughgarden2002,swamy2012,korilis1997}, and in communication networks as competitive /
