...
preconditioner solves in single-digit iterations, and the design adjoint again returns the gradient for all
...
single-digit. Remaining scope (\cref{sec:conclusion}): an active-set deflation that would make \emph{one}
...
solves in single-digit iterations; we develop this in \cref{sec:mc} (\cref{prop:mc}).
...
first-order head-to-head (\cref{tab:baseline}); \texttt{examples/reproduce\_mc.jl} regenerates the
multicommodity logit-SUE design (\cref{sec:mc}); and \texttt{test/runtests.jl} verifies every algorithm
component, including \cref{prop:laplacian,prop:conv,prop:adjoint,prop:mc}. The test networks are the TNTP Sioux Falls
...
the coupling is symmetric positive semidefinite and rank one per arc. For $K=1$, \cref{eq:mcjac} collapses to the
single-commodity active Laplacian $B\,\mathrm{diag}(1/t'_a)\,B^\top$ of \cref{prop:laplacian}.
\end{proposition}
\begin{proof}
...
their spread. Empirically (\cref{tab:mc}) the inner conjugate-gradient count is single digit, grows only
logarithmically in $K$, and is independent of graph size $n$; each Newton step therefore costs $O(K)$ near-linear
solves, and the equilibrium solve is near-linear in the total size $Kn$ in \emph{iterations}. The wall-clock
realizes this when the per-commodity Laplacians are well conditioned for the algebraic-multigrid factorization,
which holds for well-spread multi-OD demands (the realistic congestion-pricing regime); a commodity concentrated
on a single OD pair yields a near-singular block that falls back to a pinned direct solve---preserving
correctness and the iteration count at some wall-clock cost. A coupling-aware preconditioner that removes this
fallback is left to future work.
...
instance ($K=8$) projected-gradient tolling reduces TSTT by $2.1\%$ with each equilibrium re-solve costing
single-digit ($\approx6$) inner iterations---the entire bilevel iteration inheriting the near-linear per-step
cost.
...
size $n$ ($K=8$, spread). Single-digit, $\log K$ growth, $n$-independent.}
\label{tab:mc}
...
become non-unique and the block preconditioner's spread grows, and a study over real OD-matrix instances (e.g.\
TNTP city networks) remain open. Highly concentrated single-OD commodity blocks are near-singular for the
multigrid factorization and fall back to a pinned direct solve; a coupling-aware (distribution-matrix)
preconditioner that removes this fallback is the natural refinement. (ii) \textbf{Corpus scaling.} The near-linear claim is shown on one size-scaled
