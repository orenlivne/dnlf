...
tolls in one solve. Code is available at \url{https://github.com/orenlivne/dnlf}.
\end{abstract}
...
(\cref{tab:mcdesign}), and \texttt{examples/reproduce\_mc.jl} a single multicommodity design instance
(\cref{sec:mc}); and \texttt{test/runtests.jl} verifies every algorithm component, including
\cref{prop:laplacian,prop:conv,prop:adjoint,prop:mc}. The test networks are the TNTP Sioux Falls
instance (shipped with the code) and public Internet AS / P2P-overlay graphs (e.g.\ \texttt{as-caida},
...
equilibrium across all $\gamma$---the regime the warm-started design loop stays in (\cref{tab:mcdesign}); it is
only the \emph{cold} forward solve at small $\gamma$, far from equilibrium where per-commodity flows concentrate
and the blocks become high-contrast, that degrades toward the deterministic multicommodity limit ($\gamma\to0$).
We verified that a coupling-aware distribution-matrix preconditioner realized in flow space (the per-arc coupling
inverted exactly by Sherman--Morrison, with an aggregate-congestion Laplacian on the mean commodity mode, as
\cite{brandt1984} prescribes) converges $\gamma$-boundedly where the reduced-potential-space version does not,
but it does not improve on the block diagonal in the operating regime; the block diagonal is therefore retained
and the deterministic limit left open (\cref{sec:conclusion}). Fixed $\gamma$ is thus both the modelling choice
(a calibrated route-choice dispersion) and the regime in which the block preconditioner is well conditioned.
...
instance projected-gradient tolling reduces TSTT while converging toward the marginal-cost (Pigouvian) toll that
prices essentially every congested arc, with each equilibrium re-solve costing single-digit inner iterations---the
bilevel iteration inheriting the near-linear per-step cost. Across a range of instances (varying $K$, $\gamma$,
network size, and demand; \cref{tab:mcdesign}) the reduction is $1$--$2\%$ and the inner-iteration count stays
single digit. A budget-constrained (sparse-toll) design is a natural extension.
...
size $n$, and demand): TSTT reduction, inner conjugate-gradient iterations per equilibrium re-solve, and the
fraction of arcs tolled. Near-full tolling is the marginal-cost (Pigouvian) optimum; the inner count stays single
digit throughout. Regenerated by \texttt{scripts/design\_study\_mc.jl}.}
\label{tab:mcdesign}
\begin{tabular}{rrrrrr}\toprule
$n$ & $K$ & $\gamma$ & TSTT red.\ & inner it.\ & arcs tolled\\\midrule
$800$     & $4$  & $0.10$ & $1.1\%$ & $3$ & $96\%$\\
$800$     & $8$  & $0.10$ & $1.1\%$ & $4$ & $96\%$\\
$800$     & $16$ & $0.10$ & $1.2\%$ & $7$ & $94\%$\\
$800$     & $8$  & $0.15$ & $1.3\%$ & $3$ & $96\%$\\
$800$     & $8$  & $0.10$ & $1.4\%$ & $6$ & $97\%$\\
$1{,}200$ & $8$  & $0.10$ & $1.4\%$ & $6$ & $97\%$\\
$1{,}600$ & $8$  & $0.10$ & $1.2\%$ & $6$ & $98\%$\\\bottomrule
\end{tabular}
\end{table}
...
dispersion $\gamma$; the \emph{deterministic} multicommodity limit ($\gamma\to0$), where per-commodity flows
become non-unique and the cold-solve iteration count grows, and a study over real OD-matrix instances (e.g.\
TNTP city networks) remain open. We investigated the coupling-aware distribution-matrix preconditioner that
would target this limit (\cref{sec:mc}): its flow-space realization converges $\gamma$-boundedly, confirming the
construction, but it does not improve on the block diagonal in the operating regime, so a preconditioner that is
strictly better toward $\gamma\to0$ is still open. (ii) \textbf{Corpus scaling.} The near-linear claim is shown on one size-scaled
