...
iteration table (\cref{tab:mc}) and \texttt{examples/reproduce\_mc.jl} the multicommodity design
...
fallback for the rare inputs on which the multigrid factorization throws. The iteration count degrades as
$\gamma\to0$---there the per-commodity flows concentrate, the blocks become high-contrast, and the count rises
into the hundreds by $\gamma\approx0.05$; this deterministic multicommodity limit, and a coupling-aware
preconditioner robust to it, are open (\cref{sec:conclusion}). Fixed $\gamma$ is thus both the modelling choice
...
instance ($K=8$) projected-gradient tolling reduces TSTT by $2.1\%$, converging toward the marginal-cost
(Pigouvian) toll that prices essentially every congested arc, with each equilibrium re-solve costing single-digit
($\approx6$) inner iterations---the bilevel iteration inheriting the near-linear per-step cost. A single-instance
demonstration; a budget-constrained (sparse-toll) design and a multi-instance study are natural extensions.
