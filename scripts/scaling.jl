...
#
# Usage:  AIER_DATA=~/code/data julia --project=. scripts/scaling.jl [graph1.mtx graph2.mtx ...]
# Data:   SuiteSparse/SNAP .mtx graphs under $AIER_DATA (default ~/code/data).
using DNLF, NLF, LinearAlgebra, SparseArrays, Printf, Random

datadir() = get(ENV, "AIER_DATA", joinpath(homedir(), "code", "data"))
...
const DEFAULT = ["SNAP__as-735","SNAP__p2p-Gnutella08","SNAP__Oregon-1","SNAP__ca-HepPh","SNAP__ca-AstroPh",
...
# run the corpus study only when executed as a script (so other scripts can `include` the loaders)
